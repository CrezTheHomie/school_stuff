        ...
        if(input[i] == hintString[j]){
            ...
            j++;
               if(j == hintString.size()){
                //cout << "found combination due to " << input[i-3] << input[i-2] <<input[i-1] << "\n";
                combination++;
                j = tempArray[j-1];
          	}
        }
...
void lcs2( char *X, char *Y, int m, int n ){
   ...
   while (i > 0 && j > 0){
      ...
      if (X[i-1] == Y[j-1]){
          lcs[index-1] = X[i-1] - '0'; // Put current character in result
...
int main(){
    ...
    for(int i = 0; i < N; i++){
        c = combRow[i] + '0';
        //cout << "int1 = " << combRow[i] << "\n";
        //cout  << c << " "; //this statement prints out the combination found using rows
        char1[i] = c;
    }
cout << "\n";
    for(int i = 0; i < N; i++){
        c = combCol[i] + '0';
        //cout << "int2 = " << combCol[i] << "\n";
        //cout << c << " "; //this statement prinst out the combination found using columns
        char2[i] = c;
    }
cout << "\n";
	//cout << char1 << "\n";
